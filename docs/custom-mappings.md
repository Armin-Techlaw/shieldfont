...
Neither column changes the SEO reality either: encoded text is `aria-hidden` decoy in the DOM, so search engines index the decoy and you can't tell Googlebot from an AI scraper. **Don't wrap content you want ranked** (a small noun-only mapping limits, but does not eliminate, this). Copy-paste yields the encoded form, and screen readers skip protected regions rather than voice a decoy: in React, `<Shield>`'s `a11y` prop puts a real alternative beside the hidden block (`mode: "text"` ships the words encrypted in the page for the reader to unlock, through a control that is screen-reader-only by default, or `mode: "audio"` for a recording — never a link, since that would hand your words to any scraper that follows it; the costs, including the JavaScript requirement and the focus indicator a sighted keyboard user loses, are in [`plain-text-mode.md`](./plain-text-mode.md)), and outside React you have to set `aria-hidden` and supply that alternative yourself. That is friction we accept; see [`docs/integration.md`](./integration.md) for the full v1 threat model.
